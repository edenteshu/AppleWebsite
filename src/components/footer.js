  ...
  useEffect(() => {
    return () => {
      ...
      $(window).on("resize", function () {
        if ($(window).width() <= 768) {
          console.log("window widthn is less than 768");
          $(".footer-links-wrapper").addClass("someClass");
      ...
      $(document).on("click", ".someClass h3", function () {
        $(this).next("ul").slideToggle(); // Corrected from slideToogle to slideToggle
        $(this).toggleClass("expanded"); // Corrected from toogleClass to toggleClass
      });
    };
  }, []);
  return (
